(() => {
  ...
  function renderLive(payload, serverTime){
    ...
    const displaySoc = soc === null ? null : (soc >= 100 ? 100 : Math.floor(Math.max(0, soc) * 10) / 10);
    $('batterySoc').textContent = displaySoc === null ? '--%' : `${displaySoc.toFixed(1)}%`;
    $('batteryHeroSoc').textContent = displaySoc === null ? '--%' : `${displaySoc.toFixed(1)}%`;
