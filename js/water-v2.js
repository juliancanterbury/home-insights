(() => {
  ...
  const settings = (() => { try { return { range:'90', usageRate:null, serviceDaily:null, ...JSON.parse(localStorage.getItem(settingsKey) || '{}') }; } catch { return { range:'90', usageRate:null, serviceDaily:null }; } })();
  ...
  function intervals() {
    ...
    for(let i=1;i<rows.length;i++){
      ...
      const litres=units*1000,dailyLitres=litres/days;
      result.push({start,end,days,units,litres,dailyLitres,dailyCost:configured()?dailyLitres/1000*Number(settings.usageRate)+Number(settings.serviceDaily):null});
    }
    return result;
  }
  function dailyRows(){
  ...
  function renderReadings(){
    ...
    $('waterReadingList').innerHTML=rows.length?rows.map(row=>{const photo=row.hasPhoto||row.source==='manual-with-photo';const audit=row.ocrOriginal?` · OCR: ${row.ocrOriginal}`:row.ocrAttempted?' · OCR: no result':'';return `<div class="gas-reading-row${photo?' has-photo':''}"><div>${photo?`<button type="button" class="meter-photo-thumb is-loading" data-meter-photo="${row.id}" aria-label="Open saved meter photo"></button>`:''}<strong>${row.value.toLocaleString('en-AU',{maximumFractionDigits:4})} kL</strong><small>${fmtDate(row.time)} · corrected/manual${audit}</small></div><div><button data-water-edit="${row.id}">Edit</button><button data-water-delete="${row.id}">Delete</button></div></div>`}).join(''):'<div class="gas-empty"><strong>No water readings yet</strong><span>Drop a water meter photo or add a reading manually.</span></div>';
    window.HomeInsightsLocalData?.hydratePhotoThumbnails($('waterReadingList'));
    document.querySelectorAll('[data-water-delete]').forEach(button=>button.addEventListener('click',async()=>{if(!confirm('Delete this water meter reading?'))return;await window.HomeInsightsLocalData?.deleteReadingPhoto(button.dataset.waterDelete);localStorage.setItem(meterKey,JSON.stringify(readAll().filter(row=>row.id!==button.dataset.waterDelete)));window.HomeInsightsLocalData?.renderMeters();window.dispatchEvent(new CustomEvent('homeinsights:meters-changed'));}));
    document.querySelectorAll('[data-water-edit]').forEach(button=>button.addEventListener('click',()=>{const all=readAll(),row=all.find(item=>item.id===button.dataset.waterEdit),next=prompt('Correct the cumulative water meter reading (kL):',row?.value);if(next===null||!row)return;const value=Number(next.trim().replace(',','.'));if(!Number.isFinite(value)||value<0)return alert('Enter a valid meter reading.');row.value=value;row.correctedReading=value;localStorage.setItem(meterKey,JSON.stringify(all));window.HomeInsightsLocalData?.renderMeters();window.dispatchEvent(new CustomEvent('homeinsights:meters-changed'));}));
  }
  ...
  function start(){
    ...
    window.addEventListener('homeinsights:meters-changed',()=>{render();window.dispatchEvent(new CustomEvent('homeinsights:water-data-ready'));});window.addEventListener('storage',event=>{if(event.key===meterKey||event.key===settingsKey)render();});render();
  }
